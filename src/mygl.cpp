...
#define multi_threads
        ...
        PhotonMapper* pm = new PhotonMapper(numPhotons, allPhotons, &scene, sampler->Clone(0), recursionLimit);
        pm->Render();
#endif
       ...
       std::vector<Photon>* allPhotons = new std::vector<Photon>();
        ...
        qDebug() << "Bouncing Finished!";
        qDebug() <<"Num Photon= " << allPhotons->size();
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            case NAIVE_LIGHTING:
                ...
                break;
            case PHOTON_MAPPING:
                rt = new PhotonMapper(tileBounds, &scene, sampler->Clone(seed), &kdTree, recursionLimit);

                //NEW
            case BIDIRECTIONAL_LIGHTING:
                rt = new BiDirectionalIntegrator(tileBounds, &scene, sampler->Clone(seed), recursionLimit);
                break;
...
void MyGL::slot_SetIntegratorType(int t)
{
    ...
    case 1:
        ...
        integratorType = BIDIRECTIONAL_LIGHTING;
        break;
    case 2:
